...
} _dht_data = { .cb_t=NULL, .userdata_t=NULL, .cb_h=NULL, .userdata_h=NULL };
...
void temperature_dht_task(void *_args)
{
   ...
   while (1) {
      if (dht_read_data(SENSOR_TYPE, DHT_GPIO, &humidity, &temperature) == ESP_OK) {
         ...
         if(_dht_data.cb_h) {
            _dht_data.cb_h(humidity / 10.0, _dht_data.userdata_h);
         }
      }
      else {
      }
...
{
   xTaskCreate(temperature_dht_task, "dht", 2048, &_dht_data, 2, NULL);
}
